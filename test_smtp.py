...
# ==================== ROUTE IMPORT (DENGAN MULTI FORMAT) ====================
        ...
        try:
            ...
            if filename.endswith('.csv'):
                ...
                for idx, line in enumerate(lines[1:], start=2):
                    ...
                    if not nim or not nama or nim in existing_nims:
                        error_count += 1
                        continue
                    
                    existing_data.append({
                        'nim': nim, 'nama': nama, 'email': email, 'no_hp': no_hp,
                        'prodi': prodi, 'semester': int(semester) if semester.isdigit() else 1,
            ...
            # ====== FORMAT EXCEL ======
                ...
                try:
                    ...
                    for idx, row in df.iterrows():
                        nim = str(row.get('nim', '')).strip() if pd.notna(row.get('nim')) else ''
                        nama = str(row.get('nama', '')).strip() if pd.notna(row.get('nama')) else ''
                        email = str(row.get('email', '')).strip() if pd.notna(row.get('email')) else ''
                        no_hp = str(row.get('no_hp', '')).strip() if pd.notna(row.get('no_hp')) else ''
                        prodi = str(row.get('prodi', '')).strip() if pd.notna(row.get('prodi')) else 'Teknik Informatika'
                        semester = row.get('semester', 1) if pd.notna(row.get('semester')) else 1
                        ipk = row.get('ipk', 0) if pd.notna(row.get('ipk')) else 0
                        ...
                        existing_data.append({
                            'nim': nim, 'nama': nama, 'email': email, 'no_hp': no_hp,
                            'prodi': prodi, 'semester': int(semester) if str(semester).isdigit() else 1,
                            'ipk': float(ipk) if ipk else 0,
                            'tgl_daftar': datetime.now().strftime("%d-%m-%Y %H:%M")
                        })
                        existing_nims.append(nim)
                    ...
                    flash("❌ Install pandas dan openpyxl: pip install pandas openpyxl", "error")
                    return redirect(request.url)
            ...
            # ====== FORMAT JSON ======
                ...
                if isinstance(data_json, list):
                    for item in data_json:
                        nim = item.get('nim', '').strip()
                        nama = item.get('nama', '').strip()
                        email = item.get('email', '').strip()
                        no_hp = item.get('no_hp', '').strip()
                        prodi = item.get('prodi', 'Teknik Informatika')
                    ...
                    flash("❌ Format JSON harus berupa array/list", "error")
                    return redirect(request.url)
            ...
            # ====== FORMAT TXT ======
                ...
                for line in lines:
                    if not line.strip() or line.startswith('#'):
                        continue
                    # Format: nim|nama|email|no_hp|prodi|semester|ipk
                    if '|' in line:
                        values = line.strip().split('|')
                    else:
                        values = line.strip().split('\t')
                ...
                flash("❌ Format file tidak didukung. Gunakan: .csv, .xlsx, .xls, .json, .txt", "error")
                return redirect(request.url)
...
@app.route("/mahasiswa/export/<format>")
@login_required
def export_mahasiswa(format):
    data = baca_mahasiswa()
    ...
    if format == 'csv':
        ...
        writer.writerow(['NIM', 'Nama', 'Prodi', 'Semester', 'IPK', 'Email', 'No HP', 'Tgl Daftar'])
        ...
        response.headers['Content-Disposition'] = 'attachment; filename=data_mahasiswa.csv'
    ...
    # ====== EXPORT EXCEL ======
        ...
        try:
            ...
            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                df.to_excel(writer, index=False, sheet_name='Mahasiswa')
            response = make_response(output.getvalue())
            response.headers['Content-Disposition'] = 'attachment; filename=data_mahasiswa.xlsx'
    ...
    # ====== EXPORT JSON ======
        ...
        response.headers['Content-Disposition'] = 'attachment; filename=data_mahasiswa.json'
    ...
    # ====== EXPORT PDF ======
        ...
        try:
            ...
            title_style = ParagraphStyle(
                ...
                fontSize=16,
                textColor=colors.HexColor('#003B8E'),
                alignment=1,
                spaceAfter=20
            )
            elements.append(Paragraph("Data Mahasiswa SIMDAMA", title_style))
            elements.append(Spacer(1, 10))
            
            headers = ['NIM', 'Nama', 'Prodi', 'Semester', 'IPK', 'Email', 'No HP']
            table_data = [headers]
            ...
            table = Table(table_data, colWidths=[0.9*inch, 1.2*inch, 1.0*inch, 0.7*inch, 0.7*inch, 1.2*inch, 1.0*inch])
            table.setStyle(TableStyle([
                ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#003B8E')),
            ...
            doc.build(elements)
            response = make_response(output.getvalue())
            response.headers['Content-Disposition'] = 'attachment; filename=data_mahasiswa.pdf'
            response.headers['Content-Type'] = 'application/pdf'
            return response
        except ImportError:
            flash("❌ Install reportlab: pip install reportlab", "error")
            return redirect(url_for('mahasiswa'))
